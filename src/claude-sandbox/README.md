...
| dockerSocket | Host Docker socket to expose inside the container, letting Claude drive containers you already run on the host (docker compose exec, Laravel Sail, wp-cli). Leave it at /dev/null to keep Docker out of the sandbox: the socket is root on the host, so anything that can reach it can escape the container entirely. /var/run/docker.sock covers Docker Desktop and standard Linux. | string | /dev/null |
| dockerHost | Value for DOCKER_HOST inside the container. The default is the Docker CLI's own default, and with dockerSocket left at /dev/null it resolves to nothing, which is the point. Set it to unix:///var/run/docker-proxy/docker.sock to use the filtered socket from .devcontainer/socket-proxy.yml instead — that route keeps dockerSocket at /dev/null, so the real socket is never mounted and container creation is refused. | string | unix:///var/run/docker.sock |
